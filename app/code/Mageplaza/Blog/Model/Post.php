...
class Post extends AbstractModel
{
    ...
    {
        $topicIds = $this->_getResource()->getCategoryIds($this);
        if (count($topicIds)) {
            $collection = $this->postCollectionFactory->create();
            $collection->getSelect()
                ->join(
                    ['post_category' => $collection->getTable('mageplaza_blog_post_category')],
                    'main_table.post_id=post_category.post_id AND post_category.post_id != "' . $this->getId() . '" AND post_category.category_id IN (' . implode(',', $topicIds) . ')',
                    ['position']
            ...
            $collection->addFieldToFilter('enabled', '1')->addFieldToFilter('store_ids', $this->getStoreIds());
            return $collection;
        }
